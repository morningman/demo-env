...
// ── LevelHeader ──
function LevelHeader({ number, title, subtitle, technique, color = 'primary' }) {
  const bg = color === 'accent' ? 'bg-accent' : color === 'highlight' ? 'bg-highlight text-white' : 'bg-primary';
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <span className={`inline-flex items-center gap-1.5 rounded-full border-[2.5px] border-ink px-3 py-1 text-xs font-bold uppercase tracking-widest shadow-[2px_2px_0_0_theme(colors.ink)] ${bg}`}>
          Level {number}
        </span>
        {technique && (
          <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">· {technique}</span>
        )}
      </div>
      <h2 className="mt-3 font-display text-pretty text-2xl font-bold leading-tight tracking-tight sm:text-4xl">{title}</h2>
      <p className="mt-1 text-sm font-semibold leading-snug text-muted-foreground sm:mt-2">{subtitle}</p>
    </div>
  );
}
