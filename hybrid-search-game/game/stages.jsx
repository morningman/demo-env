  ...
  return (
    <section id="stage-intro" className="relative mx-auto flex min-h-[100svh] w-full max-w-xl flex-col justify-center px-5 py-12">
      <div className="mb-6 flex items-center gap-3">
        <span className="cartoon-chip cartoon-chip-accent" style={{ transform:'rotate(-2deg)', animation:'bob 4s ease-in-out infinite' }}>
          <Icon name="sparkles" size={13} strokeWidth={2.5} /> Apache Doris
        </span>
        <span className="cartoon-chip cartoon-chip-highlight" style={{ transform:'rotate(2deg)' }}>Hybrid Search</span>
      </div>

      <h1 className="font-display text-4xl font-bold leading-[1.05] tracking-tight sm:text-6xl" style={{ textWrap:'pretty' }}>
        Find the stay<br />
        that{' '}
        <span className="inline-block -rotate-1 rounded-xl border-[3px] border-ink bg-highlight px-2 text-white shadow-[3px_3px_0_0_theme(colors.ink)]">really fits</span>.
      </h1>
      ...
      <div className="cartoon-card mt-8 p-5">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border-[2.5px] border-ink bg-primary shadow-[2px_2px_0_0_theme(colors.ink)]">
            <Icon name="radio" size={18} strokeWidth={2.5} />
          </div>
          <div className="min-w-0">
            <p className="font-display text-base font-semibold">One of these stays is the best fit. Find it.</p>
            <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground">
              Plan a weekend family trip. Walk three stages of a real search pipeline — filter, retrieve, fuse — and land on the best match.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-10 flex flex-col gap-3">
        <button type="button" onClick={start} className="cartoon-btn cartoon-btn-highlight animate-cta-attention h-14 w-full justify-center text-lg">
          Start <Icon name="arrowDown" size={20} strokeWidth={3} />
        </button>
        <p className="text-center text-xs font-bold uppercase tracking-widest text-muted-foreground">~90 seconds · mobile first</p>
      </div>
    </section>
  );
}
...
function Level1() {
  ...
  return (
    <section id="stage-level1" className="relative mx-auto w-full max-w-xl px-5 py-6 sm:py-10">
      <LevelHeader number="01" title="Narrow the field" subtitle="Exact filters turn a broad search into a clean candidate set." technique="Structured Filter" color="primary" />
